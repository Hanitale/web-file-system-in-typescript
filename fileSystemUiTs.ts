declare var thisUi:UI;

class UI{
    ROOT_ID:number;
    whoClicked: number;
    folder_to_delete_found : number;
    openFileId : number;
    ...
    constructor(fs, myHistory) {
        this.ROOT_ID = 0;
        this.fs = fs;
        this.myFs = this.fs;
        this.myHistory = myHistory;
        this.root = (this.fs.fs[0] as Folder);
        this.myRoot = this.root;
        this.initialise = this.init();
        this.currentFolder = (this.fs.fs[0] as Folder);
        thisUi = this;
...
setListeners():void {
    ...
    $('button.clearFileContent').on('click', function () {
        var fileToClear = (thisUi.fs.getItem(thisUi.openFileId) as MyFile);
    ...
    $('button.saveFileContent').on('click', function () {
        ...
        var saved = (fileToSave as MyFile).setContent($('textarea').val());
        if (saved) {
...
collapseExpand (id:number, changeHistory:boolean):void {
...
showFolderList(id:number, changeHistory:boolean):void {
    this.currentFolder = (this.fs.getItem(id) as Folder);
...
showContent(id, myHistoryLocation, changeHistory) {
    this.fs.path = '';
    ...
    let getAddressLine = this.fs.getPath(this.currentFolder.id, this.fs.fs);
    this.showAddressLine();
    let toHighLight = this.currentFolder.id;
    $('a[data-id='+toHighLight+']').attr('class','highlight');
    if (this.currentFolder.children) {
        for (let item of this.currentFolder.children) {
            let liFolder = '<span><a href="#" data-custom ="folder" data-id="' + item.id + '"><img src="folders.png"> '
                + item.name + '</a></span>';
            let liFile = '<span><a href="#" data-custom ="file" data-id="' + item.id + '"><img src="File.png"> '
                + item.name + '</a> </span>';
...
responseToUser (startMessage:string, Arg1:string, Arg2:string, endMessage:string) {
    var myResponse;
...
createFile () {
    ...
    if(id==undefined){
        ...
        thisUi.currentFolder = (thisUi.fs.findById(thisUi.myRoot, id) as Folder);
    }
...
uiDelete (id:number):void {
    ...
    if (!areYouSure) {
        ...
        this.currentFolder = (this.fs.findById(this.fs.fs[0], itemToDelete.getParentId()) as Folder);
...
open(id:number):void{
    this.openFileId = id;
    let fileToOpen = (this.myFs.getItem(id) as MyFile);
    ...
    $('.textarea').keyup(function (event) {
        if (event.keyCode == 27) {
            var saved = (fileToOpen as MyFile).setContent( $('textarea').val());
            if(saved){
                $('.textarea').css('display', 'none');}
        }
    });
...
clearFileContent(file:MyFile):void {
    $('textarea').val('');
    $('textarea').focus();
    file.setContent('');
}
